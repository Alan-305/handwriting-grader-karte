...
export function QuestionGenerateQ2APage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { draft } = await apiClient.generateQ2A(token, slug, {
      ...
      setError(err instanceof Error ? err.message : "第2問(A)の生成に失敗しました");
    } finally {
      window.clearInterval(stepTimer);
      setGenerating(false);
    }
  ...
  return (
    <div>
      ...
      <PageHeader
        title="第2問(A)の生成（自由英作文）"
        description="東大第2問(A)形式で、60〜80語の英作文設問と、立場の異なる解答例2つ・和訳・採点ポイントを生成します。"
      />
        ...
        <SafeForm className="space-y-6" onSafeSubmit={handleGenerate}>
          <Card className="space-y-4 p-6">
            <CardHeader className="p-0">
              ...
              <CardDescription className="font-ja">
                格言への賛否・未来予測・価値観の定義など。解答例は賛成/反対など方向性の異なる2パターン（各60〜80語）。
              </CardDescription>
            </CardHeader>
            ...
            <div>
              <label className="font-ja text-sm text-slate-600">
                テーマ・形式（お任せの場合は空欄または「お任せ」）
              </label>
              <Textarea
                className="mt-1 font-ja"
                rows={2}
                placeholder="例：未来予測（交通手段）、強さとは何か、沈黙は同意か、お任せ"
                value={topicHint}
            ...
            {showAdvanced && (
              <div className="space-y-2 border-t border-slate-100 pt-4">
                <p className="font-ja text-xs text-slate-500">
                  {universityName}の第2問(A)過去問から形式の目安を参照します。
                </p>
                {loadingYears ? (
          ...
          <Card className="border-blue-100 bg-blue-50/50 p-4">
            <p className="font-ja text-sm text-slate-700">
              生成の流れ：①設問+解答例2つ+和訳 → ②語数・論理の検証 → ③下書き保存
            </p>
          </Card>
