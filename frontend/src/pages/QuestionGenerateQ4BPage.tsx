...
export function QuestionGenerateQ4BPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { draft } = await apiClient.generateQ4B(token, slug, {
      ...
      setError(err instanceof Error ? err.message : "第4問(B)の生成に失敗しました");
    } finally {
      window.clearInterval(stepTimer);
      setGenerating(false);
    }
  ...
  return (
    <div>
      ...
      <PageHeader
        title="第4問(B)の生成（下線部和訳）"
        description="東大第4問(B)形式で、150〜250語の英文に下線(ア)(イ)を設け、和訳・構文解説・NG直訳例を生成します。"
      />
        ...
        <SafeForm className="space-y-6" onSafeSubmit={handleGenerate}>
          <Card className="space-y-4 p-6">
            <CardHeader className="p-0">
              ...
              <CardDescription className="font-ja">
                倒置・指示語・省略を含む下線部。下線(イ)には特定語を明らかにする追加指示を付けます。
              </CardDescription>
            </CardHeader>
            ...
            <div>
              ...
              <Textarea
                ...
                rows={2}
                placeholder="例：テクノロジーと自己、労働と休息、言語と認識、お任せ"
                value={topicHint}
            ...
            {showAdvanced && (
              <div className="space-y-2 border-t border-slate-100 pt-4">
                <p className="font-ja text-xs text-slate-500">
                  {universityName}の第4問(B)過去問から形式の目安を参照します。
                </p>
                {loadingYears ? (
          ...
          <Card className="border-blue-100 bg-blue-50/50 p-4">
            <p className="font-ja text-sm text-slate-700">
              生成の流れ：①英文+下線(ア)(イ)+設問 → ②構文・指示語の検証 → ③下書き保存
            </p>
          </Card>
