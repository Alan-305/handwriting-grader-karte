...
export function QuestionGenerateQ1BPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { draft } = await apiClient.generateQ1B(token, slug, {
      ...
      setError(err instanceof Error ? err.message : "第1問(B)の生成に失敗しました");
    } finally {
      window.clearInterval(stepTimer);
      setGenerating(false);
    }
          ...
          <Card className="space-y-4 p-6">
            ...
            <div>
              ...
              <Textarea
                ...
                rows={2}
                placeholder="例：認知科学、言語と社会、生物学と環境、お任せ"
                value={topicHint}
            ...
            <div>
              <label className="font-ja text-sm text-slate-600">
                素材となる英文（任意・貼り付けるとこの英文を空所化のベースに使用）
              </label>
              <Textarea
            ...
            {showAdvanced && (
              <div className="space-y-2 border-t border-slate-100 pt-4">
                <p className="font-ja text-xs text-slate-500">
                  {universityName}の第1問(B)過去問から形式の目安を参照します。
                </p>
                {loadingYears ? (
