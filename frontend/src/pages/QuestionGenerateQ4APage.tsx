...
export function QuestionGenerateQ4APage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const { draft } = await apiClient.generateQ4A(token, slug, {
      ...
      setError(err instanceof Error ? err.message : "第4問(A)の生成に失敗しました");
    } finally {
      window.clearInterval(stepTimer);
      setGenerating(false);
    }
          ...
          <Card className="space-y-4 p-6">
            ...
            <div>
              ...
              <Textarea
                ...
                rows={2}
                placeholder="例：AI社会がもたらす倫理的課題についての英文を200語程度で作成して"
                value={topicHint}
            ...
            {showAdvanced && (
              <div className="space-y-2 border-t border-slate-100 pt-4">
                <p className="font-ja text-xs text-slate-500">
                  {universityName}の第4問(A)過去問から形式の目安を参照します。
                </p>
                {loadingYears ? (
