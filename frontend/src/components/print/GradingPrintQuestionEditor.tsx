...
export function GradingPrintQuestionEditor({
  ...
  return (
    <CollapsiblePanel
      storageKey={`grading-print-${kind}-${result.id}`}
      title={resultLabel(result)}
      defaultOpen={defaultOpen}
      className={!included ? "opacity-60" : undefined}
      headerActions={
        <label className="flex min-h-11 cursor-pointer items-center gap-2 font-ja text-xs sm:text-sm">
          <input
            type="checkbox"
            checked={included}
