...
- 日本語記述: charLimitJa 明示。**scoringPoints** + directionCriterionJa 必須
- 選択式: 正解は一つ。誤肢2個以上は「浅く読んだら選ぶ」trap。本文キーワードの並べ替えだけで解けないこと

## 曖昧さの排除（検証合格の必須条件）
- 選択式: 正解は論理的に一つ。誤肢は**誤読 trap** か明確な用法・意味の相違
- 「どれも一応言える」肢の並べは禁止
- 日本語記述: charLimitJa 必須。**scoringPoints 2〜4個** + directionCriterionJa 必須
...
- choices, underlinedText, targetWord, charLimitJa, blankLabels: 該当時のみ
- 日本語記述問には **scoringPoints**（2〜4個）と **directionCriterionJa** を必ず付ける
...
## 出力（JSON のみ）
{
  ...
  "questions": [
    ...
    },
    {
      ...
      "charLimitJa": 80,
      "scoringPoints": [
        {"pointJa": "〜", "passageBasis": "本文の該当箇所", "pointsHint": "必須"}
      ],
...
- 日本語記述: charLimitJa 内で答えられ、**scoringPoints 2個以上**で採点可能か、directionCriterionJa があるか
...
- **日本語記述問**には必ず:
  ...
  - scoringPoints: 必須採点ポイント **2〜4個**（pointJa, passageBasis, pointsHint）
  ...
  "explanations": [
    {
      ...
      "scoringPoints": [
        {"pointJa": "必須ポイント1", "passageBasis": "本文根拠", "pointsHint": "必須"}
      ],
