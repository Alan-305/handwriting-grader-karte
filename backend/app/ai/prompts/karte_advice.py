KARTE_REGISTERED_DATA_RULES = """根拠データの厳守（最重要）:
- 入力コンテキストに「明示的に」載っている事実だけを根拠にすること。推測・一般論・経験則で数値や評価を補完しないこと。
- 使ってよい根拠: 添削セッション履歴、エラー集計、ミス傾向の時系列、面談で確定した志望校・確定事項、登録済みの共通テスト得点帯、面談記録・補足メモに書かれた内容。
- 面談記録・補足メモに書かれていない次の情報には触れないこと:
  · 模試・進研模試・全統模試・河合模試などの偏差値・得点・順位・判定
  · 学校の定期テスト・校内模試の成績
  · 登録されていない共通テスト科目の得点・得点帯
  · プロフィールにない志望校・併願校・入試区分
- 面談記録に模試等が書いてある場合のみ、その記録の範囲内で引用できる（「面談では〜とのこと」）。それ以外の偏差値言及は禁止。"""

KARTE_SYSTEM = f"""あなたは難関大学・医学部受験を専門とする進学指導のエキスパートです。
...
def _has_registered_common_test_scores(scores: dict | None) -> bool:
    if not scores:
        return False
    return any(v for v in scores.values() if v and str(v).strip())


def format_unregistered_data_warnings(interview_profile: dict | None) -> str:
    """プロフィール状態に応じた「触れてはいけない未登録情報」リスト。"""
    profile = interview_profile or {}
    lines = [
        "- 模試・進研模試・全統模試などの偏差値・得点・順位（面談記録・補足メモに明示がない限り）",
        "- 学校の定期テスト・校内模試の成績（面談記録・補足メモに明示がない限り）",
        "- プロフィールにない志望校・併願校・入試区分",
    ]
    if not _has_registered_common_test_scores(profile.get("commonTestScores")):
        lines.append("- 大学入学共通テストの得点・得点帯（未登録のため）")
    if not (profile.get("confirmedFactIds") or []):
        lines.append("- 面談で確定した事項（未登録のため、確定事項を推測で補わない）")
    return "\n".join(lines)


def format_allowed_data_sources(
    *,
    interview_profile: dict | None,
    ...
    if has_session_history:
        ...
        lines.append("- 添削セッション履歴: なし（セッションに言及しない）")

    if profile.get("targetUniversities"):
        lines.append("- 面談で確定した志望校・学部")
    if profile.get("confirmedFactIds"):
        lines.append("- 面談で確定した事項")
    if _has_registered_common_test_scores(profile.get("commonTestScores")):
        lines.append("- 登録済みの大学入学共通テスト得点帯")
    if profile.get("additionalNotes", "").strip():
        lines.append("- 面談プロフィールの補足メモ")
    lines.append("- 面談記録に書かれた相談・アドバイス（記載範囲内のみ）")
    return "\n".join(lines)
...
def build_karte_user_prompt(
    *,
    ...
    notes_block = f"\n【補足メモ】\n{notes}" if notes else ""
    allowed_sources = format_allowed_data_sources(
        interview_profile=profile,
        has_session_history=bool(session_summaries.strip() and session_summaries != "セッション履歴なし"),
    )
    unregistered_warnings = format_unregistered_data_warnings(profile)
