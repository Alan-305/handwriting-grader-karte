...
from app.ai.prompts.karte_advice import (
    ...
    format_allowed_data_sources,
    format_unregistered_data_warnings,
)

# すべてのステージで厳守する共通制約（KARTE_SYSTEM と grading-criteria に準拠）
KARTE_COMMON_RULES = f"""厳守事項:
...
def build_context_block(
    *,
    ...
    has_sessions = bool(
        ...
        and "セッション履歴なし" not in session_summaries
        and session_summaries != "（時系列データなし）"
    )
    allowed_sources = format_allowed_data_sources(
        interview_profile=profile,
        has_session_history=has_sessions,
    )
...
byArea の currentLevel / gapComment では、添削履歴・エラー集計・登録済み共通テスト得点帯・面談記録の範囲だけを根拠にすること。
模試偏差値・校内テスト・未登録の数値を根拠にした表現（例:「模試の偏差値から」）は禁止。
...
3. 面談記録・補足メモに明示がない模試・偏差値・順位・校内テスト成績を根拠にした記述。
4. 「不合格」という語の使用。
5. 集計・履歴・登録プロフィールに根拠がない断定（evidence の伴わない弱点・主張）。
...
def build_integrity_prompt(
    *,
    ...
    unregistered_block = format_unregistered_data_warnings(
        interview_profile or {"commonTestScores": common_test_scores or {}}
    )
    return (
        ...
        f"{scores_block}\n\n"
        "【触れてはいけない未登録情報（根拠に使ったら違反）】\n"
        f"{unregistered_block}\n\n"
        "【点検対象: Stage1 弱点診断】\n"
        f"{diagnosis_json}\n\n"
