...
def align_session(session_id: str):
    ...
    source_bytes = firebase.download_bytes(session["sourceImagePath"])
    if not source_bytes:
    ...
    aligned_path = f"teachers/{g.teacher_id}/sessions/{session_id}/aligned.jpg"
    firebase.upload_bytes(aligned_path, aligned_bytes)
    session_svc.update_status(session_id, "grading", alignedImagePath=aligned_path)

    return jsonify({"alignedImagePath": aligned_path})
...
def crop_session(session_id: str):
    ...
    aligned_bytes = firebase.download_bytes(aligned_path)
    ...
    for q in questions:
        ...
        for target in targets:
            cropped = crop_region(aligned_bytes, target["cropRegion"])
